...
class FeishuChannel(BaseChannel):
    ...
    def _patch_message_sync(self, message_id: str, content: str) -> bool:
        """Patch (update) an existing interactive card message synchronously using PatchMessage API.
        
        Note: UpdateMessage (PUT) does not support interactive type — PatchMessage must be used instead.
        """
        try:
            ...
            if not response.success():
                logger.error(
                    ...
                    message_id, response.code, response.msg, response.get_log_id()
                )
                return False
            logger.debug("Feishu message {} patched successfully", message_id)
            return True
        except Exception as e:
            logger.error("Error patching Feishu message {}: {}", message_id, e)
            return False

    async def send(self, msg: OutboundMessage) -> None:
        """Send a message through Feishu, including media (images/files) if present.
        
        If msg.metadata contains 'message_id', the existing interactive card message
        will be updated via PatchMessage instead of creating a new one.
        """
        if not self._client:
        ...
        try:
            ...
            update_message_id = msg.metadata.get("message_id") if msg.metadata else None
